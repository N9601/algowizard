...
import { buildDijkstraSteps } from "src/lib/engine/algorithms/pathfinding";
...
export default function DijkstraPage() {
  ...
  const savedState = useSavedVisualization<SavedPathState>({
    expectedRoute: "/visualizer/pathfinding/dijkstra",
    applyState: (saved) => {
  ...
  return (
    <>
      ...
      <AlgorithmLayout
        title="Dijkstra (Grid)"
        description="Uniform-cost search that expands nodes in order of path cost, guaranteeing the shortest path without a heuristic."
        time="O(E log V)"
        space="O(V)"
        category="Graph"
        difficulty="Medium"
        ...
        actions={
          <div className="space-y-3">
            ...
            <SaveVisualizationButton
              title="Dijkstra Grid"
              algorithmSlug="dijkstra-grid"
              route="/visualizer/pathfinding/dijkstra"
              disabled={!step}
              getPayload={() => ({
      ...
      >
        ...
        <Controls
          ...
          statusText={`Cost-guided search • Step ${Math.round(progress * (controllerRef.current?.steps.length ?? 1)) + 1}`}
          onReset={() => {
            controllerRef.current?.reset();
