"use client";

import Link from "next/link";

import Navbar from "components/visualizer/Navbar";

const cards = [
  {
    title: "Minimax",
    description: "Explore full game tree search on tic-tac-toe with optimal play.",
    href: "/visualizer/decision/minimax",
    tag: "Decision AI",
  },
  {
    title: "Alpha–Beta Pruning",
    description: "See how pruning skips branches while keeping the optimal Minimax result.",
    href: "/visualizer/decision/alpha-beta",
    tag: "Decision AI",
  },
];

export default function DecisionHub() {
  return (
    <div className="min-h-screen bg-[#050b14] text-white">
      <Navbar />
      <main className="mx-auto max-w-6xl px-6 py-12">
        <div className="mb-10 max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-[0.26em] text-white/50">
            Decision AI
          </p>
          <h1 className="mt-3 text-4xl font-bold">Game tree intuition</h1>
          <p className="mt-4 text-white/70">
            Visualize how Minimax explores tic-tac-toe and how Alpha–Beta prunes
            branches without changing the optimal move.
          </p>
        </div>

        <div className="grid gap-4 md:grid-cols-2">
          {cards.map((card) => (
            <Link
              key={card.href}
              href={card.href}
              className="group rounded-2xl border border-white/10 bg-white/[0.04] p-5 transition hover:-translate-y-1 hover:border-white/20 hover:bg-white/[0.07]"
            >
              <span className="inline-flex rounded-full border border-white/10 bg-white/[0.05] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-white/65">
                {card.tag}
              </span>
              <h2 className="mt-4 text-2xl font-semibold">{card.title}</h2>
              <p className="mt-2 text-sm text-white/70">{card.description}</p>
              <div className="mt-4 text-sm font-medium text-white/75 transition group-hover:text-white">
                Open →
              </div>
            </Link>
          ))}
        </div>
      </main>
    </div>
  );
}
