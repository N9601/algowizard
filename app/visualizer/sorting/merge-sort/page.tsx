...
export default function MergeSortPage() {
  ...
  const savedState = useSavedVisualization<SavedMergeSortState>({
    expectedRoute: "/visualizer/sorting/merge-sort",
    applyState: (saved) => {
...
/>
  ...
  onStepBack={() => controller.current?.stepBackward()}
          statusText={describeSortingStep("merge", step, array)}
          onReset={() => {
            controller.current?.reset();
            setStep(null);
            setProgress(0);
