...
export default function QuickSortPage() {
  ...
  const savedState = useSavedVisualization<SavedQuickSortState>({
    expectedRoute: "/visualizer/sorting/quick-sort",
    applyState: (saved) => {
  ...
  return (
    <>
      ...
      <AlgorithmLayout
        title="Quick Sort"
        description="Quick Sort selects a pivot element and partitions the array around it. It is extremely fast in practice but has O(n²) worst-case time complexity."
        time="O(n log n)"
        space="O(log n)"
        category="Sorting"
        difficulty="Medium"
        actions={
          <div className="space-y-3">
            <SaveVisualizationButton
              title="Quick Sort State"
              algorithmSlug="quick-sort"
              route="/visualizer/sorting/quick-sort"
  ...
  onStepBack={() => controller.current?.stepBackward()}
          statusText={describeSortingStep("quick", step, array)}
          onReset={() => {
            controller.current?.reset();
            setStep(null);
            setProgress(0);
