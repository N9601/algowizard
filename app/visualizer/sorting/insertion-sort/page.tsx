...
export default function InsertionSortPage() {
  ...
  const savedState = useSavedVisualization<SavedInsertionSortState>({
    expectedRoute: "/visualizer/sorting/insertion-sort",
    applyState: (saved) => {
  ...
  return (
    <>
      ...
      <AlgorithmLayout
        ...
        description="Insertion Sort builds the sorted array one element at a time by inserting each new element into its correct position. It is efficient for small or nearly sorted datasets but has O(n²) worst-case time complexity."
        time="O(n²)"
        space="O(1)"
        category="Sorting"
        difficulty="Easy"
  ...
  onStepBack={() => controller.current?.stepBackward()}
          statusText={describeSortingStep("insertion", step, array)}
          onReset={() => {
            controller.current?.reset();
            setStep(null);
            setProgress(0);
        ...
        <ColorLegend />
        <Pseudocode algorithm="insertion" />
      </AlgorithmLayout>
    </>
  );
}
